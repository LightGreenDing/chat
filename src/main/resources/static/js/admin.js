...
layui.define(['jquery', 'form', 'layer', 'element'], function (exports) {
    ...
    $(function () {
        ...
        setTimeout(function () {
            ...
            var userMenu = 'menu_' + userName;
            var userCurMenu='curMenu_'+userName;
            ...
            if (sessionStorage.getItem(userCurMenu)) {
                $('.layui-tab-title').find('layui-this').removeClass('layui-class');
                curMenu = JSON.parse(sessionStorage.getItem(userCurMenu));
                id = curMenu.id;
                if (id) { //因为默认桌面首页不存在lay-id,所以要对此判断
                    var $m = $('.layui-tab-title li[lay-id="' + id + '"]');
    ...
    var tab = {
        toConsole: function () {
            ...
            var $iframe = $('.layui-tab-content iframe').eq(0);
            $iframe.parent().addClass('layui-show');
            var url = $iframe.attr('_src');
            if (url) {
                $iframe.attr("src", url);
            }
        },
        tabAdd: function (title, url, id) {
            ...
            if (li > 0) {
                ...
                element.tabAdd('wenav_tab', {
                    title: title,
                    content: '<iframe tab-id="' + id + '" frameborder="0" _src="' + url + '" scrolling="yes" class="weIframe"></iframe>',
                    id: id
                });
                //当前窗口内容
    ...
    function setStorageMenu(title, url, id) {
        ...
        var userName = parse.userName;
        var userMenu = 'menu_' + userName;
        var menu = JSON.parse(sessionStorage.getItem(userMenu));
        if (menu) {
            var deep = false;
            for (var i = 0; i < menu.length; i++) {
                if (menu[i].id == id) {
    ...
    function setStorageCurMenu() {
        ...
        var userName = parse.userName;
        var userCurMenu='curMenu_'+userName;
        var curMenu = sessionStorage.getItem(userCurMenu);
        var text = $('.layui-tab-title').find('.layui-this').text();
        text = text.split('ဆ')[0];
        var $iframe = $('.layui-tab-content').find('.layui-show').find('.weIframe');
        var url = $iframe.attr('src');
        if (!url) {
            url = $iframe.attr('_src');
            $iframe.attr('src', url)
        }
        ...
        sessionStorage.setItem(userCurMenu, JSON.stringify(curMenu));
    }
     ...
     * 判断初次登录时，跳转到登录页
     */
    var c = localStorage.getItem('login');
